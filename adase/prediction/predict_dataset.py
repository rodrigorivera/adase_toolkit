...
def _arima_models(df_test: pd.DataFrame,
                  target: str,
                  model_name: str,
                  model
                  ) -> pd.DataFrame:
...
def _regression_models(df_test: pd.DataFrame,
                       future_flag: int,
                       target: str,
                       model_name: str,
                       fitted_model
...
def predict_dataset(df_test: pd.DataFrame,
                    target: str,
    ...
    for future_flag in df_test.future_flag.unique():
        for item_code in df_test.item_code.unique():
            ...
            ctest = (df_test['item_code'] == item_code) & (df_test['future_flag'] == future_flag)
            ...
            if (len(df_test[ctest].values) != 0) and model is not None:
                ...
                if 'arima' in model_name:
                    ...
                    results = _regression_models(df_to_test, future_flag, target, model_name, model)
                    list_df.append(results)
