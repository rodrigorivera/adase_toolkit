...
def get_latest_directory(path: str):
    ...
    logging.debug('Entered get_latest_directory')
    full_path = os.path.join(path, datetime.datetime.now().strftime('%Y/'))
    logging.debug('get_latest_directory / full_path: {}'.format(full_path))
    temp_path = full_path + '*/'
    ...
    logging.debug('get_latest_directory / temp_path: {}'.format(latest_dir))
    list_of_directories = get_all_files(latest_dir)
    logging.debug('get_latest_directory / list_of_directories: {}'.format(list_of_directories))
    latest_directory = max(list_of_directories, key=os.path.getctime) + '/'
    logging.debug('get_latest_directory / latest_directory: {}'.format(latest_directory))
