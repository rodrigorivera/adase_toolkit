...
def create_rolling_statistics(df: pd.DataFrame,
    ...
    """
    TODO: Consider how to make code more concise
    ...
    df_grouped_mean = df_grouped\
        .transform( lambda x: x.shift(width_adjust).fillna(0).rolling(window=width).mean())\
        .fillna(0)\
        .rename(columns=cols_mean)

    df_grouped_min = df_grouped\
        .transform(lambda x: x.shift(width_adjust).fillna(0).rolling(window=width).min())\
        .fillna(0)\
        .rename(columns=cols_min)

    df_grouped_max = df_grouped\
        .transform(lambda x: x.shift(width_adjust).fillna(0).rolling(window=width).max())\
        .fillna(0)\
        .rename(columns=cols_max)

    df_grouped_std = df_grouped\
        .transform(lambda x: x.shift(width_adjust).fillna(0).rolling(window=width).std())\
        .fillna(0)\
        .rename(columns=cols_std)

    df_grouped_sum = df_grouped\
        .transform(lambda x: x.shift(width_adjust).fillna(0).rolling(window=width).sum())\
        .fillna(0)\
        .rename(columns=cols_sum)
    df_grouped_median = df_grouped\
        .transform(lambda x: x.shift(width_adjust).fillna(0).rolling(window=width).median())\
        .fillna(0)\
