...
def create_cumulative_statistics(df: pd.DataFrame, targets: List[str], condition: List[str]) -> pd.DataFrame:
    """
    It creates cumulative statistics, one new column for each of the statistics such as mean, min, max,
    std, sum, median. In total 6 for each target
    TODO: See if it is possible to refactor for a loop or something more concise
