...
def _arima_models(df_train: pd.DataFrame, target: str, model_name: str):
    """
...
def _regression_models(df_train: pd.DataFrame,
                       future_flag: int,
                       target: str,
                       model):
    """
...
def train_dataset(df_train: pd.DataFrame,
                  target: str,
                  model,
                  model_name:str
                  )-> Dict:
    ...
    df_temp = df_train\
        .set_index(['item_code', 'future_flag'])\
        .sort_index()
    ...
    for item_code in list_items_code:
        for future_flag in df_temp.loc[item_code].index.unique().tolist():
            ...
            if 'arima' in model_name:
                ...
                fitted_model = _regression_models(df_to_train, future_flag, target, model)
